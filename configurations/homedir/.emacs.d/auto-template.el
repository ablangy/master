...
(defvar auto-template-dir (if (file-exists-p (concat normal-path "templates"))
			      (concat normal-path "templates/")
			    (concat obj-path "templates/"))
...
(defvar auto-template-function-alist
  '((tpl-file-name  . (file-name-nondirectory (buffer-file-name)))
    ...
    (tpl-user-name  . (user-group-full-name))	;; with group.el
